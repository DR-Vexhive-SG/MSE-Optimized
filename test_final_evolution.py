...
# Crear DB explícitamente
pattern_db = MarketPatternDatabase(db_path=db_path)
initial_count = len(pattern_db.stored_patterns)
initial_crist = sum(1 for p in pattern_db.stored_patterns if p.crystallized)
print(f"🤖 Inicio: {initial_count} patrones, {initial_crist} cristalizados")

# Inicializar bot con DB explícita
bot = TradingBotAutonomous(symbol='BTC-USD', use_ai=True, use_patterns=True, pattern_db=pattern_db)
...
for sym in markets:
    ...
    for i, p in enumerate(prices):
        ...
        try: sig, conf, meta = bot.process_state(state, sym)
        except: continue
    ...
    curr = len(bot.pattern_db.stored_patterns)
    crist = sum(1 for p in bot.pattern_db.stored_patterns if p.crystallized)
    effs = [p.confidence for p in bot.pattern_db.stored_patterns]
    avg_eff = np.mean(effs) if effs else 0.5
    modif = [p.pattern_type for p in bot.pattern_db.stored_patterns if abs(p.confidence-0.5)>0.001]
    wr = (wins/trades*100) if trades>0 else 0
    
    print(f"   Trades:{trades} WinRate:{wr:.1f}% Patrones:{curr}(+{curr-initial_count}) Crist:{crist} Eff:{avg_eff:.3f} Modif:{len(modif)}")
    results.append({'market':sym,'trades':trades,'wins':wins,'win_rate':wr,'patterns':curr,'cristalized':crist,'avg_eff':float(avg_eff),'modified':len(modif)})
...
new_db = MarketPatternDatabase(db_path=db_path)
new_bot = TradingBotAutonomous(symbol='BTC-USD', use_ai=True, use_patterns=True, pattern_db=new_db)
final_count = len(new_bot.pattern_db.stored_patterns)
final_crist = sum(1 for p in new_bot.pattern_db.stored_patterns if p.crystallized)
final_effs = [p.confidence for p in new_bot.pattern_db.stored_patterns]
final_eff = np.mean(final_effs) if final_effs else 0.5
...
with open('data/test_results/evolution_analysis.json','w') as f:
    json.dump({'initial':{'patterns':initial_count,'cristalized':initial_crist},'final':{'patterns':final_count,'cristalized':final_crist,'avg_eff':float(final_eff)},'markets':results,'conclusion':concl}, f, indent=2)
